...
int main(int argc, char *argv[]) {
     ...
     */ 
    typedef double (*fptr)(Image *, Image *);
    fptr distFunc;
    if (strncmp(dist_metric, "euclidean", strlen(dist_metric)) == 0){
        distFunc = distance_euclidean;      
    } else if (strncmp(dist_metric, "cosine", strlen(dist_metric)) == 0){
        distFunc = distance_cosine;
    } else{
        fprintf(stderr, "distance function does not exist");
        exit(1);
    }
    ...
    int start_index = 0;
    int numProcess = ceil((testing->num_items)/num_procs);
    ...
    for (int i = 0; i < num_procs; i++){
        ...
        if (pid < 0){
            ...
            close(fd1[i][1]);
            close(fd2[i][0]);
            child_handler(training, testing, K, distFunc, fd1[i][0], fd2[i][1]);
            close(fd1[i][0]);
            close(fd2[i][0]);
            exit(0);
        }
        close(fd1[i][0]);
        close(fd2[i][1]);
        if (write(fd1[i][1], &start_index, sizeof(int)) != sizeof(int)){
            perror("write start index to pipe");
            exit(1);
        };
        
        start_index += numProcess;
        if (write(fd1[i][1], &numProcess, sizeof(int)) != sizeof(int)){
            perror("write num_process to pipe");
            exit(1);
        };
    ...
    for (int i = 0; i < num_procs; i++){
            ...
            if (WIFEXITED(status)){
                int num;
                if (read(fd2[i][0], &num, sizeof(int)) != sizeof(int)){
                    perror("read result from pipe");
                    exit(1);
                };
