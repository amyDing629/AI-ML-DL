...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    if (read(p_in, &start_idx, sizeof(int)) != sizeof(int)){
        perror("read start_idx from parent pipe");
        exit(1);
    };
    if (read(p_in, &N, sizeof(int)) != sizeof(int)){
        perror("read N from parent pipe");
        exit(1);
    };
    int rst = 0;
    for (int i=start_idx; i<start_idx + N && i<testing->num_items; i++){
        int predict = knn_predict(training, testing->images + i, K, fptr);
        if (predict == testing->labels[i]){
            rst++;
        }
    }
    if (write(p_out, &rst, sizeof(int))!=sizeof(int)){
        perror("write correct prediction to parent pipe");
        exit(1);
    };
...
double distance_cosine(Image *a, Image *b){
    ...
    return (2 / M_PI) * acos(ab / (sqrt(a2) * sqrt(b2)));
}
